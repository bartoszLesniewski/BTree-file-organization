...
class BTree:
    ...
    def insert_into_node(self, record, node):
        if not node.is_leaf():
            ...
            # if after insertion the key to child, there is no new child (not split), we don't have to modify parent
            if new_child_pointer:
                # otherwise we have to add new key from child to parent
                if len(node.records) < 2 * self.d:
                    ...
                    # new_child_pointer = None  # return ??

                # parent is full
                else:
                    # maybe add record and pointer?
    ...
    def find_position(self, node, record):
    ...
    def try_compensation(self, node, parent_node, i, record):
        can_compensate = False
        i = parent_node.pointers.index(node.page_number)
        if i - 1 >= 0 and node.is_leaf():
            left_neighbour = self.filesHandler.load_index_page(parent_node.pointers[i - 1])
            if len(left_neighbour.records) < 2 * self.d:
                self.compensation(left_neighbour, node, parent_node, i - 1, record)
                can_compensate = True
        if not can_compensate and i + 1 < len(parent_node.pointers) and node.is_leaf():
            right_neighbour = self.filesHandler.load_index_page(parent_node.pointers[i + 1])
            if len(right_neighbour.records) < 2 * self.d:
                self.compensation(node, right_neighbour, parent_node, i, record)
                can_compensate = True
